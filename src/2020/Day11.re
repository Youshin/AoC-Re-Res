...
// Some처리 후 None 
module FillSeats = (LookMethod: LookMethod) => {
  let input =
  ...
  let solve = () => {
    input->LookMethod.fillSeats->LookMethod.countOccupied->Js.log;
  };
};

module Lookaround: LookMethod = {
  ...
  let rec fillSeats = data => {
    let matrix =
      data->Array.mapWithIndex((i, element_row) => {
        let new_row =
          element_row->Array.reduceWithIndex([||], (cols, element_col, j) => {
            switch (element_col) {
            | EMPTY =>
...
module Answer1 = FillSeats(Lookaround);
Answer1.solve();
module Answer2 = FillSeats(LookDeeper);
Answer2.solve();
