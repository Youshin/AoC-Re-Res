open Belt
let input = Node.Fs.readFileAsUtf8Sync("./src/input/day5.txt")->Js.String2.split("\n");
...
let min_arr = (arr) =>
  ArrayLabels.fold_left(~f=min, ~init=max_int, arr);
...
*/
let rec takeAction = (word, curr: matrix) => {
    switch word {
        | "B" => {
            let low = avg(curr.row.hi, curr.row.lo)->ceil->int_of_float;
            { 
                row: {
                    lo: low, 
                    hi: curr.row.hi
        ...
        | "R" => {
            ...
            {col: {lo: low, hi: curr.col.hi}, row:curr.row}
        }
        | "L" => {
            let hi = avg(curr.col.hi, curr.col.lo)->floor->int_of_float;
            {col: {lo: curr.col.lo, hi: hi}, row:curr.row}
        }
...
let parse = (words) => {
    words->Array.reduce(start, (pos, data) => {
...
let findMySeat = (seats) => {
    seats->Array.keep(x => !seats->Js.Array2.includes(x+1))->Array.getExn(0)+1
}
let part1 = input
